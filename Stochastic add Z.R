...
stoch_imp <- function(data) {
  ...
  fit <- with(stochastic_imp, lm(Y ~ x_fit + Z))
...
plot(df_fnl$prob_f, df_fnl$true_var, col="black", pch="o",ylim = c(0, 0.01), xlab="Probability of missing X with Z = 1", ylab= expression(paste(hat(beta[1]), " variance")), lty=1)
...
legend(0.01,0.0095,legend=c("True Variance","Deterministic Imputation","Stochastic Imputation","Bootstrap","Complete Case"), col=c("black","orange","skyblue","#009E73","#CC79A7"),
